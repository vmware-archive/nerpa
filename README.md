...
- device_id, integer, unique identifies for the target P4 device
...
* Device-specific relations: It is possible to update relations specifically for a device. To do this, include a field called `client_id` of type `int` in an `output` relation. This field's value will be the UUID of the corresponding Client relation in OVSDB. By default, all relations corresponding to `packet_in` structs in the P4 program will have this field, because packets should be re-sent to a specific device.